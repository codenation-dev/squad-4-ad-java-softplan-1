...
public class LogService {
  ...
  List<LogListGroupedDTO> listLogsGrouped(User user, Pageable pageable, LogFilterParams filter) {
    ...
    JPAQuery<?> query = queryFactory.from(l);
    pageable.getSort().forEach(sort -> {
      this.sortPaths.forEach(path -> {
    ...
    List<Tuple> result = query
      .groupBy(l.id.count(), l.logLevel, l.code, l.message, l.createdAt.dayOfYear(), 
        l.createdAt.year())
      .where(qdslFilters)
      .limit(500)
      .select(l.id.count(), l.logLevel, l.code, l.message, l.createdAt.dayOfYear(), 
        l.createdAt.year())
      .orderBy(l.logLevel.asc())
      .fetch();
    // @formatter:on
    List<LogListGroupedDTO> out = result.stream().map(line -> {
      ...
      group.setDoy(line.get(4, Integer.class));
      group.setYear(line.get(5, Integer.class));
      return group;
  ...
  public LogService(LogRepository logRepository, ClientRepository clientRepository, MapperService modelMapperService) {
